...
/* 
 * Names: Rachel Jordan + Maegan Brown
 *
 * Loader
...
{
    return (line.length() >= COMMENT && line.at(COMMENT) == '|');
}
...
void Loader::loadLine(std::string line)
{
    ...
    uint32_t address = convert(line, ADDRBEGIN, (ADDREND - ADDRBEGIN) + 1);

    for (int i = DATABEGIN; line[i] != ' '; i += 2)
    {
        uint8_t byteValue;

        byteValue = convert(line, i, 2);
        Memory::getInstance()->putByte(byteValue, address, imem_error);
...
int32_t Loader::convert(std::string line, int32_t start, int32_t len)
{
    ...
    return std::stoul(substring, nullptr, 16);
}
...
bool Loader::hasErrors(std::string line)
{
   ...
   //   to lastAddress
        int32_t conAddress = convert(line, ADDRBEGIN, (ADDREND - ADDRBEGIN) + 1);
        if(conAddress <= lastAddress)
        {
            return true;
        }
        lastAddress += numDBytes;
   ...
   //         and addr returned by convert
        if(conAddress + numDBytes > MEMSIZE)
        {
            return true;
        }
...
bool Loader::errorData(std::string line, int32_t & numDBytes)
{
   ...
   if(hexnum%2 == 0 && isSpaces(line, firstSp, COMMENT -1))
   {
       numDBytes = hexnum/2;
   }
   else
...
bool Loader::errorAddr(std::string line)
{
    ...
    if(line.at(ADDRBEGIN - 1) != 'x' || line.at(ADDREND + 1) != ':'
        || line.at(ADDREND + 2) != ' ') 
    {
        return true;
    }
...
bool Loader::goodFile(std::string filename)
{
    ...
    return (filename.length() >= 4 && filename.find(".yo\n"));
}
